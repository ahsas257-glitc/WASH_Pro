...
def _resolve_tool_page_file(selected_tool: str) -> str | None:
    ...
    # ✅ exact naming style you have
    exact = f"Tool_{n}.py"
    if (PAGES_DIR / exact).exists():
        return exact

    # fallback styles (just in case)
...
# ============================================================
if login_clicked:
    # ✅ DO NOT modify widget keys manually (selected_tool / tpm_id)
    # but we can lock a copy for tools/pages if needed:
