...
# ============================================================
st.set_page_config(page_title="Tool 5 — WASH Report Generator", layout="wide")

# Robust project root: /pages/Tool_5.py -> project root is parents[1]
...
wiz = Wizard(WizardConfig(tool_name="Tool 5", steps=STEPS, key_prefix="tool5"))

topbar(
    title="Tool 5 — Report Generator",
    subtitle="Auto-filled from Google Sheet + SurveyCTO attachments",
    right_chip="WASH • UNICEF",
)

# MUST stay exactly "Tool 5" for Google Sheet lookup
tool_name = "Tool 5"
...
def _load_tool5_row(sheet_id: str, tool: str, tpm_value: str, tpm_col: str):
    return get_row_cached(sheet_id, tool, tpm_id=tpm_value, tpm_col=tpm_col)


row = _load_tool5_row(GOOGLE_SHEET_ID, tool_name, str(tpm_id), TPM_COL)
if not row:
    status_card("TPM not found", "The selected TPM ID was not found in the Tool 5 worksheet.", level="error")
    st.stop()
...
def _generate_docx() -> bool:
    ...
    component_observations = st.session_state.get("tool5_component_observations_final")
    if component_observations is None:
        component_observations = st.session_state.get("component_observations", [])

    severity_by_no = st.session_state.get("tool5_severity_by_no") or {}
    severity_by_finding = st.session_state.get("tool5_severity_by_finding") or {}
    add_legend = bool(st.session_state.get("tool5_add_legend", True))

    conclusion_payload = st.session_state.get("tool5_conclusion_payload") or {}
    ...
    st.session_state["tool5_docx_bytes"] = docx_bytes
    return True
...
if step == 7:
    ...
    st.session_state.setdefault("tool5_severity_by_no", {})
    st.session_state.setdefault("tool5_severity_by_finding", {})
    st.session_state.setdefault("tool5_add_legend", True)
...
if step == 8:
    ...
    payload = st.session_state.get("tool5_conclusion_payload") or {}
    ...
    st.session_state["tool5_conclusion_payload"] = {
        "conclusion_text": txt,
