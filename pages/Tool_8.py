...
# Robust project root: /pages/Tool_8.py -> project root is parents[1]
...
# MUST stay exactly "Tool 8" for Google Sheet lookup
tool_name = "Tool 8"
...
row = _load_tool8_row(GOOGLE_SHEET_ID, tool_name, str(tpm_id), TPM_COL)
if not row:
    status_card("TPM not found", "The selected TPM ID was not found in the Tool 8 worksheet.", level="error")
    st.stop()
...
def _generate_docx() -> bool:
    ...
    component_observations = st.session_state.get("tool8_component_observations_final")
    if component_observations is None:
        component_observations = st.session_state.get("component_observations", [])

    severity_by_no = st.session_state.get("tool8_severity_by_no") or {}
    severity_by_finding = st.session_state.get("tool8_severity_by_finding") or {}
    add_legend = bool(st.session_state.get("tool8_add_legend", True))

    conclusion_payload = st.session_state.get("tool8_conclusion_payload") or {}
...
if step == 7:
    ...
    st.session_state.setdefault("tool8_severity_by_no", {})
    st.session_state.setdefault("tool8_severity_by_finding", {})
    st.session_state.setdefault("tool8_add_legend", True)
...
if step == 8:
    ...
    payload = st.session_state.get("tool8_conclusion_payload") or {}
    ...
    st.session_state["tool8_conclusion_payload"] = {
        "conclusion_text": txt,
