...
# Stable form id (avoid accidental changes)
...
def _key(field: str, suffix: str) -> str:
    """
    Short + stable Streamlit key.
    IMPORTANT: include suffix to avoid collisions between widgets for same field.
    """
...
def _init_state() -> None:
    ...
    ss.setdefault("general_info_date_formats", {})   # per-field date format override
    ss.setdefault("general_info_cost_currency", {})  # per-field currency
    ss.setdefault("general_info_cost_amount", {})    # per-field amount
    ss.setdefault("_t6_step2_saved", False)          # UI flag
...
# -----------------------------------------------------------------------------
# Widgets (NO saving here — save only on submit)
# -----------------------------------------------------------------------------
...
def w_af_phone(field: str, ctx: Tool6Context) -> str:
    ...
    left, right = st.columns([0.7, 2.3], gap="small")
    with left:
...
def w_money(field: str, ctx: Tool6Context) -> str:
    """
    Store amount + currency in session_state, but DO NOT write overrides here.
    We return final string; caller saves on submit only.
    """
...
# -----------------------------------------------------------------------------
# Main render (FAST: saves only on submit)
# -----------------------------------------------------------------------------
def render_step(ctx: Tool6Context) -> bool:
    ...
    with st.container(border=True):
        ...
        # ✅ form = no rerun on each keypress (much faster)
        with st.form(_FORM_ID, clear_on_submit=False):
            ...
            with tabs[3]:
                ...
                st.divider()
                st.markdown("##### Available documents on site")
